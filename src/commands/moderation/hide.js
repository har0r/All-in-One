...
module.exports = {
  data: new SlashCommandBuilder().setName('hide').setDescription('Hide a channel from @everyone')
  ...
  async execute(interaction, client) {
    ...
    try {
      await channel.permissionOverwrites.edit(interaction.guild.roles.everyone, { ViewChannel: false }).catch(e=>{ throw e; });
      const ok = new ContainerBuilder().setAccentColor(0x5865F2)
        .addTextDisplayComponents(new TextDisplayBuilder().setContent(`## ${builders.emojis.hide} Channel Hidden`))
      ...
      const err = builders.buildErrorContainer('Hide Failed', e.message.slice(0,1500));
