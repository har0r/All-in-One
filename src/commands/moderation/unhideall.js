...
module.exports = {
  data: new SlashCommandBuilder().setName('unhideall').setDescription('Unhide all channels')
  ...
  async execute(interaction, client) {
    ...
    for (const ch of interaction.guild.channels.cache.values()) {
      try {
        await ch.permissionOverwrites.edit(interaction.guild.roles.everyone, { ViewChannel: null }).catch(()=>{ throw new Error(); });
        done++;
      } catch { failed++; }
    }
    const ok = new ContainerBuilder().setAccentColor(0x57F287)
      .addTextDisplayComponents(new TextDisplayBuilder().setContent(`## ${builders.emojis.unhide} Unhide All`))
      .addSeparatorComponents(new SeparatorBuilder().setSpacing(SeparatorSpacingSize.Small).setDivider(true))
      .addTextDisplayComponents(new TextDisplayBuilder().setContent(`**Visible:** \`${done}\` channels\n**Failed:** \`${failed}\``))
      .addSeparatorComponents(new SeparatorBuilder().setSpacing(SeparatorSpacingSize.Small).setDivider(true))
