...
module.exports = {
  data: new SlashCommandBuilder().setName('unhide').setDescription('Unhide a channel')
    .addChannelOption(o=> o.setName('channel').setDescription('Channel to unhide').setRequired(false))
    .setDefaultMemberPermissions(PermissionFlagsBits.ManageChannels),
  async execute(interaction, client) {
    await interaction.deferReply({ flags: MessageFlags.IsComponentsV2 }).catch(()=>{});
    const channel = interaction.options.getChannel('channel') || interaction.channel;
    try {
      await channel.permissionOverwrites.edit(interaction.guild.roles.everyone, { ViewChannel: null }).catch(e=>{ throw e; });
      const ok = new ContainerBuilder().setAccentColor(0x57F287)
        .addTextDisplayComponents(new TextDisplayBuilder().setContent(`## ${builders.emojis.unhide} Channel Visible`))
      ...
      const err = builders.buildErrorContainer('Unhide Failed', e.message.slice(0,1500));
